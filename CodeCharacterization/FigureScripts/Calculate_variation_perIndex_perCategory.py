...
def mask_unsupported_harmonization(df, label):
    ...
    for algo in df['Algorithm'].unique():
        ...
        if label == "Initial guess":
            supported = uses_init

        elif label == "Bounds":
            supported = uses_bounds

        elif label == "Bounds + Initial guess":
            supported = uses_bounds and uses_init
...
output_excel = (
    f"AlgorithmCategory_Variability_SNR{SNR}_corrected_implementation.xlsx"
)
...
for parameter_name, cv_col in parameter_map.items():
    ...
    filename = (
        f"CV_by_Category_"
        f"{parameter_name}_corrected_implementation.png"
    )
...
for parameter_name, cv_col in parameter_map.items():
    ...
    filename = (
        f"Global_CV_Distribution_{parameter_name}_corrected_implementation.png"
    )
