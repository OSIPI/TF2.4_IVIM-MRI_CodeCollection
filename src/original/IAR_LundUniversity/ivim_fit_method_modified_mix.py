...
class IvimModelVP(ReconstModel):
    ...
    def fit(self, data, bounds_de=None):
        ...
        # clip the initial guess for the least squares to be within the bounds, otherwise the optimization will fail
        x_f[0] = np.clip(x_f, bounds[0][0], bounds[1][0])[0]
        x_f[1] = np.clip(x_f, bounds[0][1], bounds[1][1])[1]
        x_f[2] = np.clip(x_f, bounds[0][2], bounds[1][2])[2]
